...
Como não estamos utilizando nenhum servidor de DNS nesse tutorial, precisaremos ajustar esse nome no arquivo +/etc/hosts+ da nossa máquina +base+. Então, façamos isso:
...
Observe as diferenças desse arquivo em relação ao arquivo +httpd.conf.2+ para saber o que foi editado:
[source,bash]
----
diff httpd.conf{,.2}
----
