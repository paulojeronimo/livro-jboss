...
[source,bash]
----
$ ssh jboss-tunnel-vm-centos
----

Então, com as configurações acima, não é mais necessário informar o usuário e os parâmetros para a criação dos túneis. Eles são criados automaticamente ao utilizar o alias (+jboss-tunnel-vm-centos+) como parâmetro para o +ssh+. legal, não é?
...
[source,bash]
----
$ ssh -f -N jboss-tunnel-vm-centos
----
...
[source,bash]
----
$ alias jboss-tunnel-vm-centos='ssh -f -N jboss-tunnel-vm-centos'
$ jboss-tunnel-vm-centos
$ ps -ef | grep ssh
$ pkill ssh
----
...
Existem várias alternativas para isso ... boas é más. Porém, uma delas é colocar o Apache HTTPD como _frontend_ para o JBoss. Faremos isso agora.
