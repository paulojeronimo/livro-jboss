...
Edite o script copiado (+/etc/init.d/jboss+) através de um editor de textos (Vim). Para fazer as mudanças, baseie-se na saída do comando +diff+ entre o arquivo original e o editado:
[source,bash]
----
diff $JBOSS_HOME/bin/init.d/jboss-as-standalone.sh /etc/init.d/jboss
25a26
> JBOSS_USER=aluno
28c29
...
Vamos editar a configuração do novo _virtual host_ em +httpd.conf+:
[source,bash]
----
vim httpd.conf
----

Observe as diferenças no arquivo original (+httpd.conf.2+) com o editado +httpd.conf+ para saber o que foi modificado:
[source,bash]
----
diff httpd.conf{.2,}
991c991
